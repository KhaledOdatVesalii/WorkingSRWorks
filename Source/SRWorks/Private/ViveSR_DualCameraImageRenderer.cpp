...
AViveSR_DualCameraImageRenderer::AViveSR_DualCameraImageRenderer()
{
	...
	root_arrow = CreateDefaultSubobject<UArrowComponent>(TEXT("Arrow"));
	RootComponent = root_arrow;
...
void AViveSR_DualCameraImageRenderer::SetMode(DualCameraDisplayMode mode)
{
	...
	if (image_plane != nullptr) {
		...
		UE_LOG(LogTemp, Warning, TEXT("Image Plane is Null"));
	}
}
...
void AViveSR_DualCameraImageRenderer::UpdateUndistortedPose()
{
	...
	if (IsValid(vr_origin))
	{
		...
		RootComponent->AddLocalTransform(FTransform(pose_rotation, pose_position * 100));
	}
	else
	{
		RootComponent->SetWorldLocationAndRotation(pose_position * 100, pose_rotation);
	}
}
...
void AViveSR_DualCameraImageRenderer::UpdateDistortedPose()
{
	...
	local_location = local_location * 100;

...
public class SRWorks : ModuleRules
{
    public SRWorks(ReadOnlyTargetRules Target) : base(Target)
	{
        ...
        if (Target.Platform == UnrealTargetPlatform.Win64)
        {
            ...
            PublicSystemLibraryPaths.Add(SRWorksDirectory);
            ...
            for (int i = 0; i < dlls.Length; i++)
            {
                ...
                RuntimeDependencies.Add(Path.Combine(SRWorksDirectory, dlls[i]));
            }
            string[] axs = Directory.GetFiles(SRWorksDirectory, "*.ax");
            for (int i = 0; i < axs.Length; i++)
            {
                RuntimeDependencies.Add(Path.Combine(SRWorksDirectory, axs[i]));
            }
        }
